extern "C"{
    ...
    __global__ void correlateSharedCol(unsigned char* input, unsigned char* out, 
            int inp_rows, int inp_cols, int filter_cols, int maxDown, int maxRight
    ) {
        // make filter size constant?, or use extern shared
    ...
    __global__ void correlateSharedRow(unsigned char* input, unsigned char* out, 
            int inp_rows, int inp_cols, int filter_rows, int maxDown, int maxRight
    ) {
        // make filter size constant?, or use extern shared
        ...
        float sum = 0;
        for (int filterRow = 1; filterRow < filter_rows; filterRow++) {
